...
const declarationsArray = [['РЕГИОНАЛЬНЫЙ ЭТАП ВСО 2022', 'declarationvso22reg'], 
...
function getTransfering() {
  ...
  let declarationAddress = '../DT/' + declarationsArray [keysArray.indexOf(studEvent)][1] + '.html';
...
'data[grafa2-12]', 'data[grafa8-12]', 'data[grafa9-12]', 'data[grafa14-12]', 'data[grafa18-1]', 'data[grafa21-1]', 
...
'data[grafaB-2]', 'data[grafaB-3]', 'data[grafaB-4]', 'data[grafaB-5]'];
var inputArray2 = ['data[grafa2-4]', 'data[grafa8-4]', 'data[grafa9-4]', 'data[grafa14-4]', 'data[grafa2-1]', 'data[grafa8-1]', 'data[grafa9-1]'];

[].forEach.call(inputElements,function(el){
  el.addEventListener('focusout', function (e) {
    el.value = el.value.toUpperCase();
    if (inputArray1.includes(el.name)) {
      el.value = el.value.replace(/\s/g,'');
    } else if (inputArray2.includes(el.name)) {
      el.value = el.value.trim();
      if (el.value) {
        el.value = el.value.split('.')[0] + '.' + el.value.split('.')[1].trim();
      } 
    } else {
      el.value = el.value.trim();
    }
