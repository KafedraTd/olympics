...
document.addEventListener("DOMContentLoaded", () => {
            ...
            for (var i = 0; i<loginArray.length; i++) {
                ...
                if ((loginUsername == usrName) || (loginUsername == usrLogEm)) {
                    ...
                    if (loginPassword == usrPsw) {
                        if (usrName == 'админ'){
                            window.open('menuUser.html', '_self');
                        } else {
                            window.open('menuUser.html', '_self');
                        }
                        transfering(usrName);
    ...
    formCreate.addEventListener("submit", e => {
        ...
        // fetch(url, {method: 'POST', body: new FormData(formCreate)
        // }).then(response => alert ("Thanks!")).catch(error => console.error('Error!', error.message));
    });
...
localStorage.setItem('logiName', x);
}
